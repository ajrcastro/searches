...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    ucsPQ = util.PriorityQueue()
    currState = problem.getStartState() 
    priority = heuristic(currState, problem)
    ucsPQ.push((currState, [], 0), priority)
    ...
    while True:
        ...
        if currState not in closedSet:
            ...
            currSuccessors = problem.getSuccessors(currState)
            for successor in currSuccessors: 
                cost = newNode[2] + successor[2]
                priority = cost + heuristic(successor[0], problem)
                if successor[0] not in closedSet:
                    ucsPQ.push((successor[0], newNode[1]+[successor[1]], cost), priority)
    ...
    while not (problem.isGoalState(currState)):
        ...
        for successor in currSuccessors:
            if successor[0] not in closedSet:
                priority = heuristic(successor[0], problem) + problem.getCostOfActions(successor[1])
                astarPQ.push((successor, successor[1] + nextAction, priority,), priority)
                closedSet.add(successor[0])
        nextNode = astarPQ.pop()
        nextAction = nextNode[1]
        currState = nextNode[0]
        if astarPQ.isEmpty() and not problem.isGoalState(currState):
            return []
    return nextNode[1]
    """
